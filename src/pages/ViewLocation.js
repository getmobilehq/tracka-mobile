...
const ViewLocation = () => {
  ...
  const { data, loading, error } = useLocationContext();
  const { toggleDrawer } = useContext(SidebarContext);

  const { handleChangePage, totalResults, resultsPerPage, dataTable } =
    useFilter(data);

  const currLocation = dataTable.find(
    (data, index) => data.name.split(" ").join("_").toLowerCase() === location
  );

  const centers = currLocation?.centers;
        ...
        <TableContainer className="mb-8">
          <Table>
            ...
            <LocationSubmitContextProvider>
              <LocationDetailsTable centers={centers} />
            </LocationSubmitContextProvider>
          </Table>
